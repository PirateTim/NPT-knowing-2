...
**Version:** 4.0 (ARB Submission Draft)
...
*   **Schema Enforcement:** We will utilize the native LangGraph tables (`checkpoints`, `checkpoint_blobs`, `checkpoint_writes`). However, to ensure records can be located by agent and thread, Hook must configure the checkpointer to inject a strict metadata JSON payload on every save:
    ```json
    {
      "agent_name": "[String: e.g., Spyglass]",
      "thread_id": "[String: Unique execution thread identifier]"
    }
    ```
...
**Schema 1: `cognitive_lens.json`**
Defines the agent's core worldview and vocabulary constraints.
```json
{
  "agent_name": "string",
  "domain_bias": "string (Explicit instructions on how this agent evaluates data)",
  "strict_lexicon": {
...
**Schema 2: `learned_rules.json`**
An append-only log of direct corrections and new guidelines provided by the user or peer agents.
```json
[
  {
    "timestamp_utc": "ISO-8601 string",
    "rule_directive": "string (The explicit correction or new rule)",
    "source_context": "string (e.g., 'User correction in Issue #14')"
  }
]
```

**Schema 3: `few_shot_exemplars.json`**
Concrete examples of desired input/output behavior to anchor the model's structural generation.
```json
[
  {
    "exemplar_id": "string",
    "input_context": "string (The raw data or prompt provided to the agent)",
    "ideal_output": "string (The exact structural response expected)",
...
## 6. ARB DECISION REQUIRED: Interactive Design Phase Initiation

**USER ACTION REQUIRED:** Please review the two options below for how Hook should initiate the "Interactive Design Phase" (Phase 1) when you request a new agent. Delete the option you reject.

### [OPTION 1] The Structured Questionnaire Approach
**Description:** When the user requests a new agent (e.g., "I need an agent named Spyglass"), Hook is mandated to respond with a strict, numbered questionnaire asking for the Domain Bias, Lexicon, and Tool Requirements before proceeding.
**Pros:** Guarantees all fields for the `cognitive_lens.json` and tool manifest are captured immediately.
**Cons:** Can feel rigid and overly bureaucratic if the user just wants to brainstorm.

### [OPTION 2] The Conversational Discovery Approach (Recommended)
**Description:** Hook engages in a natural, conversational deliberation. She is instructed to organically extract the Domain Bias, Lexicon, and Tool Requirements over multiple turns. She will only present the final structured blueprint for sign-off once she believes she has gathered enough context.
**Pros:** Aligns with FR-1.2 (Conversational Competency) and allows for fluid brainstorming and architectural debate.
**Cons:** Requires Hook to accurately track missing requirements across a longer context window.