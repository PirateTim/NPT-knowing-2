...
*   **Principle 4: Absolute Ingestion Provenance:** All ingested data must be captured in full and tagged with verifiable metadata, including a cryptographic hash, to ensure an unbroken chain of custody.
...
*   **Principle 7: Native Cloud Ecosystem Containment:** The system will operate entirely within the Google Cloud Platform (GCP) ecosystem, utilizing native SDKs (`google-genai`) to ensure security and stability.
...
3.  **Phase 3: Automated Provisioning:** Hook provisions the agent's file structure, including its persistent memory vault, and registers the new agent in the system's `infrastructure_manifest.json`. The agent is then available for use by the runtime engine.
...
*   **Storage:** A dedicated directory is created for each agent at `src/react_agent/project_knowledge/agent_memory_vault/[Agent_Name]/`.
...
*   **Provenance:** The cryptographic hash and other source metadata will be stored as **GCS Object Metadata** (`x-goog-meta-source-uri`, etc.), providing the benefits of both readability and verifiable lineage.
...
## 6. ARB DECISION REQUIRED: Checkpoint Schema Conflict
...
### Option A: Fully Custom Schema

*   **Source:** `docs/npt_fleet_design.md`
*   **Description:** Create a bespoke table (`npt_agent_state_store`) with explicit columns like `active_crew_member`, `is_verified_by_grog`, etc.
*   **Pros:**
    *   Makes agent identity a first-class citizen in the database schema.
    *   Data is highly structured and directly queryable.
*   **Cons:**
    *   **Incompatible with LangGraph's native `PostgresSaver` checkpointer.** This would require writing, testing, and maintaining a completely custom checkpointer, re-implementing features like time-travel debugging from scratch. This introduces significant engineering overhead and risk.

### Option B: Native LangGraph Schema
...
*   **Cons:**
    *   Agent identity and other custom metadata are not first-class columns. They must be stored within the generic `metadata` JSONB column, which could be perceived as less structured.

### Recommended Path Forward: Hybrid Approach

**Adopt Option B (Native Schema) with a strict governance layer.**

This design recommends using the native LangGraph schema to avoid rewriting the checkpointer. However, it mandates that a **structured JSON object** with a defined schema be stored in the `metadata` column on every checkpoint.

**Mandatory Metadata Schema:**
```json
{
  "active_crew_member": "string",
  "zotero_item_key": "string",
  "is_verified_by_grog": "boolean",
  "is_challenged_by_bilgeladle": "boolean"
}
```

This hybrid approach provides the best of both worlds: it leverages the power and stability of the native framework while enforcing the required structured data for agent identity and role-specific states, satisfying all stated architectural goals.
