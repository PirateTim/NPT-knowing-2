...
# Load secure environment variables (.env cache)
load_dotenv()
...
class MCPClientManager:
    ...
    def _bootstrap_mcp_servers(self):
        ...
        for server_name, server_entry in config.get("mcpServers", {}).items():
            ...
            try:
                # Force shell execution context to perfectly resolve Node binaries under Windows paths
...
class RuntimeEngine:
    def __init__(self, profile_paths: List[str]):
        ...
        self.model_tier = "gemini-2.5-pro"
    ...
    def start_chat_loop(self):
        ...
        # UNIFIED TOOL PACKAGING: Explicitly wrap python methods and json declarations
        # to guarantee Type Uniformity for Automatic Function Calling (AFC)
        unified_tools = []
        
        # Register the local Python callables wrapped inside an explicit types.Tool block
        ...
        unified_tools.append(local_tool_block)
        
        # Register the dynamic MCP server tool definitions wrapped inside their own types.Tool block
        ...
        print(f"[INIT] Dynamically mounted {len(self.mcp_manager.tools_metadata)} sanitized MCP tools and 2 local workspace tools.")
        print("[INIT] Type 'exit' or 'quit' to close the sovereign session block.\n")
                ...
                while response.function_calls:
                    for call in response.function_calls:
                        if call.name == "read_local_file":
                            mcp_result = read_local_file(**call.args)
                        elif call.name == "write_local_file":
                            mcp_result = write_local_file(**call.args)
                        else:
                            mcp_result = self.mcp_manager.call_mcp_tool(
                                namespaced_name=call.name,
                                arguments=call.args
                            )
                        response = chat.send_message(mcp_result)
