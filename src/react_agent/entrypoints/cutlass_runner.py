"""
Cutlass Interactive & CLI Terminal (Entrypoint)
...
def run_cutlass_agent():
    parser = argparse.ArgumentParser(description="NPT Fleet: Cutlass Forensic Auditor")
    ...
    if args.prompt:
        print(f"\n[AUTHOR -> CUTLASS] ({thread_id}) -> {args.prompt}")
    ...
    while True:
        try:
            user_input = input(f"\n[AUTHOR -> CUTLASS] ({thread_id}) -> ")
            if user_input.lower() in ['exit', 'quit']:
                print(f"\n[SYSTEM] Terminating Cutlass session ({thread_id}). State saved.")
                break
